...
class WanImageToVideoBaseClass:
    @staticmethod
    def _encode_with_vae_lambda(positive, negative, vae, width, height, length, batch_size, start_image, clip_vision_output, vae_encoder_lambda):
        ...
        if start_image is not None:
            ...
            concat_latent_image = vae_encoder_lambda(vae, image)
...
class WanImageToVideo(WanImageToVideoBaseClass):
    @classmethod
    def INPUT_TYPES(s):
    ...
    def encode(self, positive, negative, vae, width, height, length, batch_size, start_image=None, clip_vision_output=None):
        vae_encoder_lambda = lambda vae, image : vae.encode(image[:, :, :, :3])
        return super()._encode_with_vae_lambda(positive, negative, vae, width, height, length, batch_size, start_image, clip_vision_output, vae_encoder_lambda)

class WanImageToVideoTiled(WanImageToVideoBaseClass):
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {"positive": ("CONDITIONING",),
                             "negative": ("CONDITIONING",),
    ...
    def encode(self, positive, negative, vae, width, height, length, batch_size, tile_size, overlap, temporal_size=64, temporal_overlap=8, start_image=None, clip_vision_output=None):
        vae_encoder_lambda = lambda vae, image: vae.encode_tiled(image[:, :, :, :3], tile_x=tile_size, tile_y=tile_size, overlap=overlap, tile_t=temporal_size, overlap_t=temporal_overlap)
        return super()._encode_with_vae_lambda(positive, negative, vae, width, height, length, batch_size, start_image, clip_vision_output, vae_encoder_lambda)
...
NODE_CLASS_MAPPINGS = {
    ...
    "WanImageToVideoTiled": WanImageToVideoTiled,
...
NODE_DISPLAY_NAME_MAPPINGS = {
    # _for_testing
    "WanImageToVideoTiled": "WanImageToVideo (Tiled VAE Encode)",
}
